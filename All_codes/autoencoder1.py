...
data_folder = '/Users/shraawanilattoo/Library/CloudStorage/OneDrive-NorthCarolinaStateUniversity/NCSU/FDS 510/Project Files/Image_data_working/Mouse wound photos-updated/Cropped images adjusted brightness'  # path to your images
dataset = ImageFolderDataset(data_folder, transform=transform)
dataloader = DataLoader(dataset, batch_size=64, shuffle=True)

# -------------------------------
# 3. RGB Convolutional Autoencoder
# -------------------------------
class ConvAutoencoderRGB(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(256, 512, 4, stride=2, padding=1),  # 4 → 2
            nn.ReLU(),
        )
...
with torch.no_grad():
    for imgs in DataLoader(dataset, batch_size=64):
        imgs = imgs.to(device)
        encoded = model.encoder(imgs)  # shape: [B, C, 4, 4]
...
'''def show_all_images_in_cluster(cluster_id):
    ...
    plt.suptitle(f"All images in Cluster {cluster_id} ({n_images} images)")
    plt.tight_layout()
    plt.show()'''

def show_all_images_in_cluster(cluster_id, n=5):
    indices = np.where(cluster_labels == cluster_id)[0][:n]  # Only first n
...
for cid in range(n_clusters):
    show_all_images_in_cluster(cid)
